...
public class A_QSort {
    ...
    private void qSort(Segment[] segments, int left, int right) {
        if (left < right && segments != null) {
            int partition = partition(segments, left, right);
            qSort(segments, left, partition - 1);
            qSort(segments, partition + 1, right);
        }
    }

    private int partition(Segment segments[], int begin, int end) {
        Segment segment = segments[end];
        int i = (begin - 1);
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        InputStream stream = new FileInputStream(root + "by/it/group973601/feshenko/lesson05/dataA.txt");
