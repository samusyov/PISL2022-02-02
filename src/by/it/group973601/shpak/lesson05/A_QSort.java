...
public class A_QSort {
    ...
    int[] getAccessory(InputStream stream) throws FileNotFoundException {
        ...
        customQuickSort(segments,0,segments.length - 1);
    ...
    private void customQuickSort(Segment[] segments,int left,int right) {
        if (left < right && segments != null) {
            int partitionIndex = partition(segments, left, right);

            customQuickSort(segments, left, partitionIndex-1);
            customQuickSort(segments, partitionIndex+1, right);
        }
    }
    private int partition(Segment segments[], int begin, int end) {
        ...
        for (int j = begin; j < end; j++) {
            if (segments[j].compareTo(pivot) >= 0) {
                ...
                Segment swapTemp = segments[i];
                segments[i] = segments[j];
                segments[j] = swapTemp;
            }
        }

        Segment swapTemp = segments[i+1];
        segments[i+1] = segments[end];
        segments[end] = swapTemp;
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        InputStream stream = new FileInputStream(root + "by/it/group973601/shpak/lesson05/dataA.txt");
